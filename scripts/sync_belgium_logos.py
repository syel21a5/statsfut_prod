...
def sync_belgium():
    ...
    for db_name, sofa_name in mapping.items():
        ...
        if sofa_id:
            ...
            updates.append(f"UPDATE betstats.matches_team SET name = '{sql_sofa_name}', api_id = 'sofa_{sofa_id}' WHERE name = '{sql_db_name}' AND league_id = 32;")

    with open('update_belgium_teams.sql', 'w', encoding='utf-8') as f:
        f.write("\n".join(updates))
    print(f"SQL gerado: {len(updates)} updates.")

    # 4. Download logos using curl_cffi to bypass 403
    base_static = os.path.join('static', 'teams', 'belgica', 'pro-league')
    ...
    for sofa_name, sofa_id in sofa_teams.items():
        ...
        # even if file exists, let's try to overwrite if it's 0 bytes
