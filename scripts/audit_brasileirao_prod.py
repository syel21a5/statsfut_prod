...
def final_audit():
    print("=== AUDITORIA CIRÚRGICA: BRASILEIRÃO 2025 (PRODUÇÃO) ===")
    ...
    print(f"\nPos | Time (ID) | API_ID | Slug | Logo Path")
    print("-" * 80)
    ...
    for s in standings:
        ...
        print(f"{s.position:2} | {t.name[:20]:20} ({t.id:4}) | {t.api_id:10} | {t.slug:20} | {status}")
    ...
    for r in remos:
        ...
        print(f" -> ID: {r.id} | Name: {r.name} | API: {r.api_id} | Slug: {r.slug} | Games: {games}")
    ...
    for b in brants:
        ...
        print(f" -> ID: {b.id} | Name: {b.name} | API: {b.api_id} | Slug: {b.slug} | Games: {games}")
