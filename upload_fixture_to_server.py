...
1. Exportar os dados da Áustria do banco local (SQLite de CI)
...
def export_and_upload():
    print("=== Exportando dados da Áustria ===")
    league = League.objects.filter(country='Austria').first()
    if not league:
        print("ERRO: Liga da Áustria não encontrada. O scraper falhou?")
        sys.exit(1)
    ...
    # SSH para limpar a Áustria antiga e importar a nova
    print("\n=== Importando dados no servidor ===")
    ssh_cmd = [
        'ssh', '-o', 'StrictHostKeyChecking=no',
        f"{server_user}@{server_host}",
        f"cd {server_path} && "
        f"{venv_python} clear_austria.py && "
        f"{venv_python} manage.py loaddata {fixture_path} && "
        f"{venv_python} manage.py recalculate_standings --division AUT && "
        f"rm {fixture_path}"
    ]
    ...
    print("\n✅ Datos da Áustria atualizados no servidor com sucesso!")
