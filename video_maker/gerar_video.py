...
def run_choreography(page, total_duration, timeline=None):
    ...
    page.evaluate("""
        ...
        style.innerHTML = `
            ...
            /* Ajustes de Enquadramento de Cinema para 1080p com borda fina */
            html, body {
...
def main():
    ...
    try:
        ...
        # 3. Gera a Thumbnail automaticamente
        import subprocess
        thumbnail_output_filename = output_filename.replace('.mp4', '_capa.png')
        thumbnail_output_path = os.path.join(output_dir, thumbnail_output_filename)
        
        print("\n[5/5] Fabricando Miniatura (Thumbnail)...")
        thumb_script = os.path.join(base_dir, "video_maker", "gerar_thumbnail.py")
        subprocess.run([sys.executable, thumb_script, "--url", match_url, "--output", thumbnail_output_path])
