...
def fetch_season_data(session, tournament_id, season_id, year_label):
    ...
    os.makedirs(f"historical_data/Finland/Veikkausliiga", exist_ok=True)
    with open(f"historical_data/Finland/Veikkausliiga/{year_label}.json", 'w', encoding='utf-8') as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(f"✅ Sucesso! {year_label}.json salvo.")
    return True

def main():
    parser = argparse.ArgumentParser(description="Busca histórico da Veikkausliiga (Finlândia) no SofaScore")
