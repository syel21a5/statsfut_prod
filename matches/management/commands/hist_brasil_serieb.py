...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do Brasileirão Série B (Brasil)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Brasil/SerieB"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='Série B', country='Brasil').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Série B' (Brasil) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='Série B', country='Brasil', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do Brasileirão Série B concluída!"))
