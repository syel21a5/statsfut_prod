...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Noruega (Eliteserien)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Norway/Eliteserien"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Eliteserien', country__icontains='Noruega').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Eliteserien' (Noruega) não encontrada no banco."))
            return
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Eliteserien', country='Noruega', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Noruega concluída!"))
