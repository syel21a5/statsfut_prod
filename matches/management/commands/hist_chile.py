...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do Chile (Primera Division)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Chile/PrimeraDivision"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Primera Division', country__icontains='Chile').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Primera Division' (Chile) não encontrada no banco."))
            return
        ...
        for year in range(2021, 2026):
             call_command('recalculate_standings', league_name='Primera Division', country='Chile', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do Chile concluída!"))
