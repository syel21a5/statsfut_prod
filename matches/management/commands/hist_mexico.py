...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do México (Liga MX)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Mexico/LigaMX"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Liga MX', country__icontains='Mexico').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Liga MX' (Mexico) não encontrada no banco."))
            return
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Liga MX', country='Mexico', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do México concluída!"))
