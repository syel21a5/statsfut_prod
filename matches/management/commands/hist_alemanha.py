...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Alemanha (Bundesliga)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Germany/Bundesliga"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='Bundesliga', country='Alemanha')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'Bundesliga' (Alemanha) criada no banco."))  # type: ignore
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='Bundesliga', country='Alemanha', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Alemanha concluída!"))  # type: ignore
