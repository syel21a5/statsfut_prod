...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Suécia (Allsvenskan)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Sweden/Allsvenskan"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Allsvenskan', country__icontains='Suecia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Allsvenskan' (Suécia) não encontrada no banco."))
            return
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='Allsvenskan', country='Suecia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Suécia concluída!"))
