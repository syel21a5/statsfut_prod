...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do Equador Serie B."

    def handle(self, *args, **options):
        base_dir = "historical_data/Ecuador/SerieB"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='Serie B', country='Equador').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Serie B' (Equador) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='Serie B', country='Equador', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do Equador Serie B concluída!"))
