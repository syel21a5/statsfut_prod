...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Polônia (Ekstraklasa)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Poland/Ekstraklasa"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Ekstraklasa', country__icontains='Polonia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Ekstraklasa' (Polonia) não encontrada no banco."))
            return
