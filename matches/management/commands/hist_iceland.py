...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Islândia (Besta deild karla)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Iceland/BestaDeildKarla"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='Besta deild karla', country='Islandia')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'Besta deild karla' (Islandia) criada no banco."))  # type: ignore
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Besta deild karla', country='Islandia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Islândia concluída!"))  # type: ignore