...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Espanha (La Liga)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Spain/LaLiga"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='La Liga', country='Espanha')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'La Liga' (Espanha) criada no banco."))  # type: ignore
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='La Liga', country='Espanha', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Espanha concluída!"))  # type: ignore
