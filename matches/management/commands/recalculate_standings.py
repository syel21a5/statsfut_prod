...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        league_name_arg = options["league_name"]
        season_year = options.get("season_year")

        if league_name_arg.upper() == "ALL":
            leagues = League.objects.all()
        else:
            leagues = League.objects.filter(name=league_name_arg)

        if not leagues.exists():
            self.stdout.write(self.style.ERROR(f"Nenhuma liga encontrada para: {league_name_arg}"))
            return
        ...
        for league in leagues:
            if season_year:
                ...
                if not season:
                    # self.stdout.write(self.style.WARNING(f"Nenhuma temporada com jogos encontrada para {league.name}"))
                    continue

            self.stdout.write(
                self.style.SUCCESS(
                    f"Recalculando tabela para {league.name} ({league.country}), temporada {season.year}"
                )
            )
            ...
            if not finished_matches.exists():
                self.stdout.write(self.style.WARNING(f"Nenhum jogo finalizado encontrado para {league.name}"))
                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Tabela recalculada para {league.name}. Times atualizados: {created}"
                )
            )
