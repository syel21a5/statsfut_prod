...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Finlândia (Veikkausliiga)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Finland/Veikkausliiga"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Veikkausliiga', country__icontains='Finlandia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Veikkausliiga' (Finlandia) não encontrada no banco."))
            return
        ...
        for year in range(2020, 2026):
             call_command('recalculate_standings', league_name='Veikkausliiga', country='Finlandia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Finlândia concluída!"))
