...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos de Portugal (Primeira Liga)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Portugal/PrimeiraLiga"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Primeira Liga', country__icontains='Portugal').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Primeira Liga' (Portugal) não encontrada no banco."))
            return
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='Primeira Liga', country='Portugal', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica de Portugal concluída!"))
