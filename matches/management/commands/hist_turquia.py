...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Turquia (Süper Lig)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Turkey/SuperLig"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Süper Lig', country__icontains='Turquia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Süper Lig' (Turquia) não encontrada no banco."))
            return
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='Süper Lig', country='Turquia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Turquia concluída!"))
