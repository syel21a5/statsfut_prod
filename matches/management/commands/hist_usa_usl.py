...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do USL Championship (Estados Unidos)."

    def handle(self, *args, **options):
        base_dir = "historical_data/USA/USL"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='USL Championship', country='Estados Unidos').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'USL Championship' (Estados Unidos) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='USL Championship', country='Estados Unidos', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do USL Championship concluída!"))
