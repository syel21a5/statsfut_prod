...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do Paraguai (Primera Division)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Paraguay/PrimeraDivision"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='Primera Division', country='Paraguai')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'Primera Division' (Paraguai) criada no banco."))  # type: ignore
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Primera Division', country='Paraguai', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do Paraguai concluída!"))  # type: ignore
