...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Colômbia (Primera A)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Colombia/PrimeraA"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='Primera A', country='Colombia')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'Primera A' (Colombia) criada no banco."))  # type: ignore
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Primera A', country='Colombia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Colômbia concluída!"))  # type: ignore
