...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos dos Estados Unidos (MLS)."

    def handle(self, *args, **options):
        base_dir = "historical_data/USA/MLS"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='MLS', country__icontains='Estados Unidos').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'MLS' (Estados Unidos) não encontrada no banco."))
            return
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='MLS', country='Estados Unidos', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da MLS concluída!"))
