...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Superettan (Suécia)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Sweden/Superettan"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='Superettan', country='Suecia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Superettan' (Suécia) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='Superettan', country='Suecia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Superettan (Suécia) concluída!"))
