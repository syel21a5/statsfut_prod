...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Argentina Primera B."

    def handle(self, *args, **options):
        base_dir = "historical_data/Argentina/PrimeraB"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='Primera B', country='Argentina').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Primera B' (Argentina) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='Primera B', country='Argentina', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Argentina Primera B concluída!"))
