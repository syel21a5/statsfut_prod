...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da 1st Division (Noruega)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Norway/1stDivision"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='1st Division', country='Noruega').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga '1st Division' (Noruega) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='1st Division', country='Noruega', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da 1st Division (Noruega) concluída!"))
