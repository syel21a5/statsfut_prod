...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Ucrânia (Premier League)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Ukraine/PremierLeague"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Premier League', country__icontains='Ucrania').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Premier League' (Ucrânia) não encontrada no banco."))
            return
        ...
        for year in range(2021, 2027):
             call_command('recalculate_standings', league_name='Premier League', country='Ucrania', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Ucrânia concluída!"))
