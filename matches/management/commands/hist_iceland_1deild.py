...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da 1. deild (Islândia)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Iceland/1deild"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='1. deild', country='Islandia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga '1. deild' (Islândia) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='1. deild', country='Islandia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da 1. deild (Islândia) concluída!"))
