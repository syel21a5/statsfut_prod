...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Ykkösliiga (Finlândia)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Finland/Ykkosliiga"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name='Ykkösliiga', country='Finlandia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Ykkösliiga' (Finlândia) não encontrada no banco."))
            return
        ...
        for year in [2024, 2025, 2026]:
             call_command('recalculate_standings', league_name='Ykkösliiga', country='Finlandia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Ykkösliiga (Finlândia) concluída!"))
