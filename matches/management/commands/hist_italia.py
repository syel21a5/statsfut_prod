...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos da Itália (Serie A)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Italy/SerieA"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))
            return

        league = League.objects.filter(name__icontains='Serie A', country__icontains='Italia').first()
        if not league:
            self.stdout.write(self.style.ERROR("Liga 'Serie A' (Itália) não encontrada no banco."))
            return
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Serie A', country='Italia', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica da Itália concluída!"))
