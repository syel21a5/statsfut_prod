...
class Command(BaseCommand):
    help = "Importa automaticamente todos os payloads JSON históricos do Equador (Liga Pro)."

    def handle(self, *args, **options):
        base_dir = "historical_data/Ecuador/LigaPro"
        
        if not os.path.exists(base_dir):
            self.stdout.write(self.style.ERROR(f"Diretório {base_dir} não encontrado!"))  # type: ignore
            return

        league, created = League.objects.get_or_create(name='Liga Pro', country='Equador')  # type: ignore
        if created:
            self.stdout.write(self.style.SUCCESS("Liga 'Liga Pro' (Equador) criada no banco."))  # type: ignore
        ...
        for year in range(2020, 2027):
             call_command('recalculate_standings', league_name='Liga Pro', country='Equador', season_year=year)

        self.stdout.write(self.style.SUCCESS(f"\n✅ Importação histórica do Equador concluída!"))  # type: ignore