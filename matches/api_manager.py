...
class APIManager:
    ...
    LEAGUE_MAPPINGS = {
        ...
        'Brasileirão Série A': {'api_football': [71], 'football_data': [2013]},
    ...
    def _get_api_football_fixtures(self, api_id, api_config, status='live', league_ids=None, days_ahead=7):
        ...
        if status == 'live':
            ...
            if days_ahead >= 0:
                # Futuro: de hoje até hoje + X dias
                date_from = today
                date_to = today + timedelta(days=days_ahead)
            else:
                # Passado: de hoje - X dias até hoje
                # days_ahead é negativo, então somar subtrai
    ...
    def _normalize_api_football_data(self, fixtures):
        ...
        for fixture in fixtures:
            normalized.append({
                ...
                'league_id': fixture['league']['id'],  # Adicionado ID da liga
    ...
    def _normalize_football_data(self, matches):
        ...
        for match in matches:
            normalized.append({
                ...
                'league_id': match['competition']['id'], # Adicionado ID da liga
