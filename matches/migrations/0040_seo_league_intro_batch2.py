# Fix: batch 2 intro texts — match by URL slug (EN country) instead of slugify(DB name)
import json
import os
...
def load_seo_texts(apps, schema_editor):
    ...
    json_path = os.path.join(settings.BASE_DIR, 'conteudo-lote2.json')
    ...
    print(f"\n[SEO] Ligas atualizadas (lote 2): {updated}")
    if missing:
        print(f"[!] Ligas do Lote 2 NÃO encontradas: {missing}")


class Migration(migrations.Migration):

    dependencies = [
        ('matches', '0039_seo_league_intro_batch1_fix'),
    ]
