# Fix: batch 4 intro texts — match by URL slug (EN country) instead of slugify(DB name)
import json
import os
...
def load_seo_texts(apps, schema_editor):
    ...
    print(f"\n[SEO] Ligas atualizadas (lote 4): {updated}")
    if missing:
        print(f"[!] Ligas do Lote 4 NÃO encontradas: {missing}")


class Migration(migrations.Migration):

    dependencies = [
        ('matches', '0041_seo_league_intro_batch3'),
    ]
