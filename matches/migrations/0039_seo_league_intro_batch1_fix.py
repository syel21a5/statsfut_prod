# Fix: batch 1 intro texts — match by URL slug (EN country) instead of slugify(DB name)
import json
import os
...
def load_seo_texts(apps, schema_editor):
    ...
    print(f"\n[SEO] Ligas atualizadas (fix): {updated}")
    if missing:
        print(f"[!] Ainda NÃO encontradas: {missing}")


class Migration(migrations.Migration):

    dependencies = [
        ('matches', '0038_merge_20260802_1404'),
    ]
